...
def get_track_df(h5_path, track_variables, group='tracks', jet_variables=None, num_jets=None):
    """
    Load flattened, valid tracks from input h5 to DataFrame. Optionally add
    repeated jet information for each track.
    """
...
def get_num_tracks_per_jet(h5_path, group='tracks', num_jets=None):
    """
    Returns the number tracks per jet. This may also be already provided by a jet variable.
    """
...
def standardise_column_names(df):
    """
    Standardise column names for downstream functions. Ensure each dataframe
    only contains predictions from a single model.
    """
    ...
    def rename_func(x):
        if x.endswith("_pb"): return 'p_b'
        elif x.endswith("_pc"): return 'p_c'
        elif x.endswith("_pu"): return 'p_u'
        elif x == 'HadronConeExclTruthLabelID': return 'flavour'
        elif x == 'n_tracks_loose': return 'n_tracks'
        elif x == 'nPromptLeptons': return 'n_truth_promptLepton'
        else: return x

    return df.rename(rename_func, axis='columns')


def load_jets(h5_path, variables, group="jets", num_jets=None, standardise_columns=True, rescale_pt=False, remove_prompt_leptons=True):
    """
